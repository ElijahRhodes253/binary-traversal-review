public class TraversalReview {
    ...
    public static int tripleSum(TreeNode node) {
        ...
        int nLeft = tripleSum(node.left);
        int nRight = tripleSum(node.right);
        return node.data * 3 + nLeft + nRight;
    }
    ...
    public static int positiveSum(TreeNode node) {
        if(node == null) return 0;
        int nLeft = positiveSum(node.left);
        int nRight = positiveSum(node.right);
        int nCenter = 0;
        if(node.data > 0)
        {
            nCenter = node.data;
        }
        return nCenter + nLeft + nRight;
    }
    ...
    public static int evenMax(TreeNode node) {
        ...
        if(node.data % 2 != 0) maxVal = Integer.MIN_VALUE;
        else maxVal = node.data;

        int leftVal = evenMax(node.left);
        int rightVal = evenMax(node.right);

        if(maxVal < leftVal) maxVal = leftVal;
        if(maxVal < rightVal) maxVal = rightVal;
    ...
    public static boolean isIncreasing(TreeNode node) {
        ...
        boolean bool2 = false;
        boolean bool3 = false;
        if(isIncreasing(node.left) && isIncreasing(node.right))
        {
            bool1 = true;
        }
    ...
    public static boolean hasZero(TreeNode node) {
        ...
        if(hasZero(node.left) || hasZero(node.right)) bool1 = true;
        if(node.data == 0) bool2 = true;
        if(bool1 || bool2) return true;
        return false;
    }
    ...
    public static boolean hasNonDivisible(TreeNode node, int k) {
        ...
        boolean bool1 = false;
        boolean bool2 = false;
        if((hasNonDivisible(node.left, k) || node.left == null) && (hasNonDivisible(node.right, k) || node.right == null))
        {   
            bool1 = true;
        }
        if(node.data % k == 0)
    ...
    public static String concatenate(TreeNode node) {
        if(node == null) return "";
        String fS = "";
        fS += concatenate(node.left);
        fS += String.valueOf(node.data);
        fS += concatenate(node.right);
        return fS;
    }
}